...
def test_화면도_팬아웃_미조회를_기본선택에서_뺀다():
    """`board.js` 의 기본 선택이 `join.selectable` 과 **같은 규약**이다 (D-08 / CR-02).

    서버가 `팬아웃미조회` 로 갈라 놔도 화면이 그걸 안 읽으면, "필터 전체 N건 선택" ·
    배너 확인 경로가 그 행을 도로 집어 크레딧을 재지불한다 — 기본 선택을 막는
    **마지막 자리**가 여기다(`board.js` 의 `고를수있는`).

    그리고 화면이 "사본 0건" 을 **단언하지 않는지**도 본다. 빈칸으로 두면 숫자 칸에서
    0 과 구분이 안 되고, `기작업` 칸이 비면 "태그가 없다" 로 읽힌다.
    둘 다 **시스템 쪽 사정**이지 광고 쪽 오류가 아니다 (Pitfall 3 의 버킷 분리).
    """
