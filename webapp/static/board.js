...
(function () {
  ...
  // 팬아웃(사본·기작업 태그)을 **못 물어본 행**의 표기 (CR-02).
  //
  // 빈칸으로 두면 안 된다: 사본 칸의 빈칸은 숫자 0 과 구분이 안 되고, 기작업 칸의
  // 빈칸은 "태그가 없다" 로 읽힌다. 둘 다 **우리가 안 본 것**이지 관측이 아니다.
  // 이건 **시스템 쪽 사정**이다 — 광고 쪽 오류(🛠)와 같은 칸에 담지 않는다(Pitfall 3).
  // 그래서 해소 칸의 시스템 기호(⚙)를 그대로 쓴다.
  var 팬아웃미조회표기 = "⚙ ?";

  function 태그(cell) {
    // 기작업 태그는 용팀장의 손자국이고 상태는 불사자의 기계 기록이다.
    // **한 값으로 합치지 마라** (Pitfall 8) — 합치면 D-08 의 "인지하고 직접 판단" 이 죽는다.
    if (cell.getRow().getData().팬아웃미조회) { return 팬아웃미조회표기; }
    var v = cell.getValue();
    if (빈칸이면(v)) { return ""; }
    return String(v);
  }

  /** 사본 N건. **못 물어본 행은 0 이 아니다** — 화면이 "사본 0건" 을 단언하지 않는다. */
  function 사본(cell) {
    if (cell.getRow().getData().팬아웃미조회) { return 팬아웃미조회표기; }
    return 정수(cell);
  }

  /** 사본·기작업 칸의 hover 전문. 왜 비었는지를 사람이 읽을 수 있어야 한다. */
  function 팬아웃전문(e, cell) {
    if (!cell.getRow().getData().팬아웃미조회) { return ""; }
    return "사본·기작업 태그를 못 물어봤다(시스템 사정 — 레이트리밋·응답 오류). "
      + "사본이 0건이라는 뜻이 **아니다.** 태그를 못 읽었으므로 이 행은 기본 선택에서 "
      + "빠진다 — 직접 체크하면 대상이 된다. 조인 스캔을 다시 돌리면 판정이 바뀐다.";
  }
   ...
   */
  /**
   * 기본 선택 대상 — 기작업이 아니고 팬아웃을 **읽은** 행만
   * (D-08 / STATE-05 · `join.selectable` 과 **같은 규약**).
   *
   * 팬아웃 미조회를 같이 빼는 이유(CR-02): 그 행은 `그룹태그` 를 못 읽어 `기작업` 이
   * 기본값 false 다. 즉 `구매_가공완료` 가 붙은 상품이 조회 실패 한 번으로 기본 선택에
   * 들어가고 Phase 5 가 **크레딧을 재지불한다.** 서버가 이미 `join.selectable` 로 막지만,
   * "필터 전체 N건 선택" 은 화면이 고르는 경로라 여기가 마지막 자리다.
   * 규약을 이 함수 **한 곳**에 둔다 — 두 벌이면 한쪽만 고쳐진다.
   */
  function 고를수있는(행들) {
    return 행들.filter(function (r) { return !r.기작업 && !r.팬아웃미조회; });
  }
  ...
  function 요약갱신(활성데이터) {
    ...
    // 기본 선택에서 빠지는 이유는 **둘이고, 한 칸에 담지 않는다** (Pitfall 3 의 정신).
    //   · 기작업        = 이미 한 일이다 (사람의 손자국 · 기계 기록)
    //   · 팬아웃 미조회 = 우리가 못 물어본 것이다 (시스템 사정)
    // 합쳐서 "기작업 N건 제외" 로 띄우면 조회 실패가 "이미 했다" 로 둔갑한다.
    var 기작업제외 = 활성.filter(function (r) { return r.기작업; }).length;
    var 팬아웃제외 = 활성.filter(function (r) {
      return !r.기작업 && r.팬아웃미조회;
    }).length;
  ...
  if (전체링크) {
    전체링크.addEventListener("click", function () {
      ...
      var 후보 = 고를수있는(활성);
      var 기작업뺌 = 활성.filter(function (r) { return r.기작업; }).length;
      var 팬아웃뺌 = 활성.filter(function (r) {
        return !r.기작업 && r.팬아웃미조회;
      }).length;
      if (!배너 || !배너말) { return; }
      배너말.textContent =
        ...
        + 콤마(대상수(후보)) + "건."
        + (기작업뺌 ? " 기작업 " + 콤마(기작업뺌) + "건은 빼고 고른다 (직접 체크하면 들어간다)." : "")
        // 조회 실패를 "이미 했다" 와 같은 문장에 담지 않는다 — 사유가 다르다.
        + (팬아웃뺌 ? " 사본·기작업 태그를 못 물어본 " + 콤마(팬아웃뺌)
            + "건도 뺀다 (시스템 사정 — 스캔을 다시 돌리면 들어온다)." : "")
        + " 진행할래?";
      배너.hidden = false;
    });
  }
  ...
  if (배너확인) {
    배너확인.addEventListener("click", function () {
      배너닫기();
      // **기작업 행과 팬아웃 미조회 행을 뺀다** (D-08 / STATE-05 / CR-02).
      // 크레딧 재지불을 막는 마지막 자리다. 판단은 `고를수있는` **한 곳**이 진다 —
      // 여기에 조건을 또 적으면 규약이 두 벌이 되고 한쪽만 고쳐진다.
      // 헤더 체크박스(`rowRange: "visible"`)는 건드리지 않는다 — 눈으로 보고 직접
      // 고르는 경로는 그대로 열려 있어야 한다.
      // 행 객체를 통째로 넘긴다 — 한 번에 넘겨야 선택 변경 이벤트가 한 번만 돈다.
      var 고를것 = {};
      고를수있는(table.getData("active")).forEach(function (r) { 고를것[r.key] = 1; });
