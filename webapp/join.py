...
_기본값 = {
    ...
    "사본N": None,
    "잠금혼재": None,
    # 팬아웃(사본·기작업 태그)을 **못 물어본 행**인가 (CR-02). 이 칸이 없으면
    # "사본 0건 · 기작업 아님" 과 "못 물어봤다" 가 화면에서 같은 값이 된다.
    "팬아웃미조회": None,
}
...
def attach(rows, result, join_doc, *, excluded=(), done_tags=()):
    ...
    판정 순서:
      ...
      5. 히트면 상태·기작업·팬아웃을 붙인다. 팬아웃을 **못 물어본 행**(`팬아웃미조회`)은
         사본 수를 `0` 이 아니라 빈칸으로 두고 기본 선택에서 뺀다 (CR-02 / D-08)
    ...
    **장수(`pages`) 인자는 없다.** 기작업 스킵은 목표 장수와 무관한 절대 조건이다
    ...
    for r in rows or []:
        ...
        # 팬아웃을 못 물어본 행인가 (CR-02). 키가 없는 **옛 산출물은 `False`** 다 —
        # 새 키가 없다는 이유로 전 행이 기본 선택에서 빠지면 화면이 통째로 못 쓰게 된다.
        # `bool()` 을 쓰지 않는다: `bool("0")` 은 True 라 문자열 "0" 이 뒤집힌다.
        행["팬아웃미조회"] = state.불리언정규화(관측.get("팬아웃미조회"))

        사본 = 관측.get("사본")
        # 🔴 **못 물어본 행에 `사본N = 0` 을 적지 않는다.** 0 은 "사본이 없다" 는
        #    판정처럼 보이는데 실제로는 안 본 것이다(`_기본값` 주석의 정신).
        #    화면이 그걸 단언하면 JOIN-03 팬아웃 경고가 통째로 죽는다.
        if isinstance(사본, list) and not 행["팬아웃미조회"]:
            행["사본N"] = len(사본)
...
def selectable(attached_rows) -> list[dict]:
    """기본 선택 대상 — 해소됐고, 기작업이 아니고, 팬아웃을 **읽은** 행만 (D-08 / STATE-05).
    ...
    **기본값 False** 다. 즉 `구매_가공완료` 가 붙은 상품이 조회 실패 한 번으로 기본
    선택에 들어가고 Phase 5 가 크레딧을 재지불한다. 태그를 못 읽은 행에 크레딧을
    태우지 않는 쪽이 안전한 기본값이다 — 기작업과 **같은 수법**으로, 목록에는 남기고
    기본 선택에서만 뺀다.
    """
    return [r for r in attached_rows or []
            if (r or {}).get("해소")
            and not (r or {}).get("기작업")
            and not (r or {}).get("팬아웃미조회")]
