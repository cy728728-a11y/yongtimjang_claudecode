...
`BULSAJA_ACCOUNT_KEY`(이 맥북의 유일한 등록 서버는 **`bulsaja` = 용쌤/부킹 계정**) 또는 `--account`.
정본은 [`_shared/불사자-안전규칙.md` §계정 라우팅](../_shared/불사자-안전규칙.md).

⚠️ 아래 2026-09-16 서술은 **이 맥북에선 더 이상 성립하지 않는다** (2026-09-19 재확인):
~~전역 환경변수 `BULSAJA_MCP_TOKEN` 이 박혀 있어 계정 이름을 무시하고 용팀장 계정으로 붙는다~~
— 셸 프로필에 그 변수가 없다. 다른 PC(윈도 데스크탑)에선 아직 유효할 수 있으니,
상품이 "없다"고 나오면 이 스킬의 `load_config_by_key()` 가 변수를 걷어내는 경로는 그대로 둔다.
