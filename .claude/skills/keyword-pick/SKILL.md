---
name: keyword-pick
description: 아이템스카우트 키워드 익스포트(.xlsx)에서 이룸님 기준(상품수 낮은 키워드)에 맞는 추천 키워드를 골라 구글 시트에 누적. "키워드 추천", "키워드 뽑아줘", "키워드 골라줘", "아이템스카우트", "상품수 낮은 키워드", "추천키워드", "keyword pick" 등을 언급하거나 아이템스카우트 익스포트 .xlsx를 제공하면 자동 실행.
allowed-tools:
  - Bash
  - Read
  - Write
...
- **폴더**: `50-소스자료 / 51-셀러라이프-통다운 / <YYMMDD>/` — **여기가 원본 보관처다**
  (2026-08-01 로컬 D: 이관). 로컬에 없으면 `eroomlib.gdrive.resolve_raw_dir()` 가
  캐시(`~/.eroom/sellerlife-cache`)로 자동 다운로드한다 (run_batch·run_names 배선 완료).
- **51 folderId**: `<workspace.toml drive.sellerlife_folder>` (하위 날짜폴더는 실행 시 생성)
- **원본 = `<YYMMDD>/raw/` 의 전체카테고리 zip (12개 대분류 통째). 지우면 안 된다.**
  `cat_prefilter` 는 상품 카테고리가 뭐가 될지 몰라 12개가 다 필요하고, zip 에서 그때그때 뽑아 쓴다.
  셀러라이프 *가공* 이 쓰는 3개(가구&인테리어·디지털&가전·생활&건강[+2])는 zip 에서 재생성되므로
  드라이브에 따로 안 올린다(2026-08-06). 개별 상품 필터는 로컬 파일로 하고, `원본파일링크`는 해당 날짜폴더 링크를 쓴다.
- **신선도 7일** — `--source-date` 를 생략하면 최신 통다운을 쓰고, 7일 초과·zip 결손이면 자동으로 새로 받는다.
  점검: `python -m eroomlib.gdrive runs`
