...
MATRIX_ROUNDTRIP_CLOSED = (
    "보류(실물기준없음·왕복종결 — 상품에 실물 사진이 없어 옵션·썸네일 어느 축도 못 푼다)")


def close_roundtrip(to_option, m):
    """옵션으로 되돌리려는 건 중 **이미 옵션이 답을 준** 것을 골라낸다 (2026-08-17).

    **왜 필요한가 — 종결 장치가 prep 경로에만 있었다** (5회차 §3① 이 발견하고 남긴 숙제).
    `prep` 은 `NO_REAL_BASE`(`실물기준없음`) 낱말을 보면 선기록을 건너뛰고 비전 배치로
    보내 왕복을 끝낸다. 그런데 **그 뒤 `verdict` 가 `제외(기준이미지없음)` 을 내면
    `TO_OPTION_VERDICTS` 경로가 다시 옵션 열로 되돌린다** — 이쪽엔 "몇 바퀴째인가"를
    보는 장치가 없다. 25-2 5회차 실측:

        썸네일 108건 → 옵션 58 → 썸네일 51 → 옵션 30 → 썸네일 24 → 옵션 26
                                                                    ↑ 3바퀴째인데 안 줄었다

    3회차 판정은 사용가능 1건 / 기준이미지없음 12 / 대표옵션의심 4 였고, 사유는 전부
    "대표옵션이 도면·배너" 또는 "후보 중에 실물이 없다" — **상품에 실물 사진이 아예 없어서
    옵션 축도 못 푸는 것**이다. 옵션이 대표를 다시 세워도 그 옵션 이미지가 또 도면이다.
    그때는 26건을 사람이 손으로 끊었다.

    **판정 근거는 옵션 열이 이미 `실물기준없음` 을 담고 있는가**다. 담고 있다면 옵션은
    이미 제 몫을 다했고(= "실물 이미지가 있는 옵션이 하나도 없다"고 답했고), 썸네일이
    후보에서 직접 고르는 경로까지 돌았는데도 안 풀린 것이다. 되돌려봐야 같은 답이 온다.
