...
def render(generated, title="썸네일 검수"):
    ...
    **대표옵션을 생성본 바로 왼쪽에 놓는다** — 이룸님이 승인할 때 판단해야 하는 것이
    ...
    """
    # 판정 도움말 — 실측 사고 유형을 눈앞에 둔다. 특히 색상코드 글자(#FEFA50 류)는
    # 디폴트 프롬프트의 hex 값이 이미지에 렌더링된 사고로 3건 관측됐다(2026-08-05).
    parts = ["<div class='meta'>판정 3축: ①제품 정확성(색·구성·스펙 표기 = 대표옵션과 "
             "같은 물건인가) ②클릭성 — <b>색상코드 등 글자 생성 여부(#FEFA50 류)</b> "
             "③오인 위험(소품·연출이 구성품처럼 보이는가)</div>"]
