...
스캔 흐름
  ...
  3. `bulsaja_market_groups` 한 번(실측 0.11초, 86개). `groupId`·`그룹명` 만 싣는다.
     **번호 추출·매칭을 여기서 하지 않는다** — 그건 웹앱(`join.market_number` /
     ...
     응답 해석은 `ss_index_calls.그룹꺼내기` 다. **못 읽었거나 0개면 exit 2 고 산출물을
     쓰지 않는다** — 빈 마켓그룹 목록은 웹앱에서 "번호가 있는 모든 행이 광고 청소 대상"
     으로 읽힌다. 조회 실패가 사람의 삭제 작업 목록이 되는 것이 이 페이즈 최대 오진이다
...
종료코드
  ...
  2  대상이 없다 (`--targets` 파일이 없거나 배열이 비었다) · 마켓그룹을 못 읽었거나 0개다
...
def 배치조회(mcp, 간격, 코드들, 재시도대기, 묶음크기):
    """`bulsaja_product_find_by_code` 를 **배치로** 돌려 항목을 모은다.

    반환 `(항목들, 실패코드)` — **못 물어본 코드를 들고 나온다** (CR-02).

    ⚠️ 행마다 한 번씩 부르지 마라. 실측상 50코드 배치가 0.53초인데, 194행을 낱개로
       부르면 194호출이 되어 레이트리밋 예산(초당 4회)의 40%를 먹는다.

    🔴 **실패를 0건으로 접지 않는다.** 예전에는 두 가지가 같은 자리에서 사라졌다:
       · 오류 응답 `{"_text": ...}` 는 dict 이고 `미조회=False` 라 가드에 안 걸렸고,
         `.get("항목") or []` 가 **로그 한 줄 없이** 0건으로 접었다
       · 429 로 소진된 배치는 경고는 찍혔지만 **행에는 아무 표시가 안 남았다**
       두 경우 모두 그 행이 `그룹태그 = None` · `사본 = []` 로 저장되고, 웹앱이 그걸
       확정값으로 읽어 **"사본 0건 · 기작업 아님"** 을 단언한다. 그러면 `구매_가공완료`
       가 붙은 상품이 조회 실패 한 번으로 기본 선택에 들어가고, Phase 5 가 크레딧을
       **재지불한다**(D-08/STATE-05 무력화). 그래서 실패한 코드를 호출부로 돌려준다.
    """
    항목들, 실패코드 = [], set()
    ...
    for i in range(0, len(코드들), 묶음크기):
        ...
        if 미조회:
            ...
            실패코드.update(조각)
            continue
        # 해석은 `ss_index_calls` 한 곳이다 (CR-01 과 같은 이유).
        try:
...
def main():
    ...
    try:
        ...
        #
        # 🔴 여기서 응답을 `or []` 로 읽으면 안 된다 (CR-01). 불사자 MCP 는 툴 레벨
        #    오류를 예외로 올리지 않는다 — `{"_text": "MCP error -32602: ..."}` 같은
        #    평범한 dict 로 준다. 그걸 "마켓그룹이 0개구나" 로 읽으면 이 스캔이 `exit 0`
        #    으로 `"마켓그룹": []` 산출물을 쓰고, 웹앱은 그 잡을 성공으로 집어 와
        #    **번호가 있는 모든 행**을 "불사자에 없는 번호" = 광고 청소 대상으로 띄운다.
        #    시스템 고장이 사람의 삭제 작업 목록으로 둔갑한다 — 되돌릴 수 없다.
        간격.대기()
        try:
            그룹응답 = 표시규칙제거(mcp.call_tool("bulsaja_market_groups", {}) or {})
            원본그룹 = 그룹꺼내기(그룹응답, 맥락=" (bulsaja_market_groups)")
        except Exception as e:
            # 트레이스백 대신 **서버가 준 문구**를 싣는다 — 다음 사람이 원인을 바로 읽는다.
            사유 = f"⛔ 마켓그룹을 못 읽었다: {e} — 산출물을 쓰지 않는다."
            말하기(사유)
            오류말하기(사유)
            return 2
        ...
        마켓그룹 = [{"groupId": str(g.get("groupId") or ""),
                   ...
                   for g in 원본그룹 if isinstance(g, dict)]

        # 키는 있는데 0개인 응답도 거부다. 그 빈 목록이 산출물에 실리면 결과가 위 오류와
        # **한 글자도 다르지 않다**(전 행 광고청소). 실측 86개이고, 마켓그룹이 진짜로
        # 0개인 계정이라면 애초에 조인할 것이 없다 — 0개는 유효한 관측이 아니다.
        if not 마켓그룹:
            사유 = ("⛔ 마켓그룹이 0개다 — 그건 관측이 아니라 조회 실패다. "
                   "산출물을 쓰지 않는다.")
            말하기(사유)
            오류말하기(사유)
            return 2
        ...
        for i, (acct, mall) in enumerate(대상, 1):
            ...
            행 = {"acct": acct, "mallProductId": str(mall),
                  ...
                  "판매자상품코드": None,
                  "불사자코드": None,
                  # **`[]` 가 아니라 `None` 이다** (CR-02). `[]` 는 "사본이 0건이었다" 는
                  # 관측처럼 보이는데 실제로는 아직 안 물어본 것이다. 웹앱이 그 둘을
                  # 구분해야 화면이 "사본 0건" 을 함부로 단언하지 않는다.
                  "사본": None,
                  # 팬아웃(사본·기작업 태그)을 **못 물어본 행**이라는 표시.
                  # 이 칸이 없어서 429·오류가 행에 한 글자도 안 남았다(CR-02).
                  "팬아웃미조회": False}
            행들.append(행)
        ...
        if 코드행:
            ...
            # 1단을 못 받은 코드의 행은 여기서 끝이다 — 묶음 키가 없어 2단에도 못 간다.
            # 그 사실을 **행에 적는다.** 안 적으면 `그룹태그=None` 이 "태그가 없다" 로
            # 읽혀 기작업(D-08) 제외가 죽는다.
            for 코드, 행들묶음 in 코드행.items():
                if 코드 in 실패1:
                    for 행 in 행들묶음:
                        행["팬아웃미조회"] = True
            ...
            if 묶음별행:
                ...
                for 묶음, 행들묶음 in 묶음별행.items():
                    # **`사본별.get(묶음, [])` 를 쓰지 않는다** (CR-02). 이 묶음 키는
                    # 1단 응답에서 나온 것이라 2단이 성공했다면 최소 1건은 돌아온다.
                    # 아무것도 안 왔다면 그건 "사본이 0건" 이 아니라 **못 받은 것**이다.
                    사본 = 사본별.get(묶음)
                    for 행 in 행들묶음:
                        행["사본"] = 사본
                        if 묶음 in 실패2 or 사본 is None:
                            행["팬아웃미조회"] = True
    finally:
        try:
            mcp.close()
    ...
    말하기(f"끝 — 행 {len(행들)} · 관측 {관측수} · 미조회 {미조회합}"
          ...
          + f" · 사본묶음 {묶음키수}건 · {소요}초")
    if 팬아웃미조회합:
        말하기(f"※ 팬아웃(사본·기작업 태그)을 못 물어본 행 {팬아웃미조회합}건 — "
              "그 행은 화면에서 '사본 0건' 이 아니라 빈칸으로 뜨고 **기본 선택에서 빠진다** "
              "(D-08: 태그를 못 읽은 행에 크레딧을 태우지 않는다). 스캔을 다시 돌려라.")
    return 0
