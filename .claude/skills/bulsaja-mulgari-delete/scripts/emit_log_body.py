# -*- coding: utf-8 -*-
# 물갈이 삭제 로그 append용 body JSON 생성.
...
# 출력: {"values":[[...20행...]]}  ← ensure_ascii=True(기본)로 한글을 \uXXXX로 인코딩(커맨드라인 깨짐 방지)
import sys, json
...
try:
    ...
    group = sys.argv[4]
    date = sys.argv[5]
    B = 20  # 배치당 상품 수 (market_delete 최대치)
    ...
    for line in seg:
        ...
        # 컬럼: 작업일시·마켓그룹·상품명·판매자상품코드·taskId·배치#·접수결과·최종결과·결과확인일시·비고·productId
        values.append([date, group, name, "", taskId, str(idx), "접수됨", "", "", "", pid])

    # ensure_ascii=True(기본): 한글이 \uXXXX ASCII로 나가 커맨드라인/코드페이지에서 안 깨짐
    print(json.dumps({"values": values}))
except Exception as e:
    # 실패 시 빈 values라도 반환하지 않고 에러를 stderr로 (호출측이 감지)
