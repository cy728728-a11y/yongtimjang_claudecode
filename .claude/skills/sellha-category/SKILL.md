...
| 옵션 | 기본 | 뜻 |
|---|:---:|---|
| `--sleep` | 5.0 | 조회 간 **최소** 대기(초) (2026-08-01 2.0→5.0 상향) |
| `--sleep-max` | 15.0 | 조회 간 **최대** 대기. 실제 대기는 이 사이 난수 (2026-08-01 `--sleep`×3→고정 15.0) |
| `--rest-every` | 10 | 평균 N건마다 **긴 휴식**(주기도 길이도 난수). 0=끔 (2026-08-01 30→10, 더 잦게) |
| `--block-wait` | 300 | **차단 감지 시** 대기(초). 그 뒤 1회 재시도 |

> **2026-08-01 실측**: 108건 배치(파일럿 10건 + 이 배치)를 옛 기본값(2~6초 간격)으로 돌리다 65번째
> 즈음부터 확장이 "네이버 보안확인" 상태에 빠지고, 66번째에서 명시적 `차단감지`(네이버 쇼핑 접속 제한)가
> 떴다. **IP 기준 차단으로 확인됨** — 브라우저·프로필을 바꿔도 우회되지 않는다. 위 기본값을 그 직후
> 상향했다. 또한 `run_all.py finish/auto` 의 `--sellha-sleep` 계열 인자가 실제로는 `sellha.py` 호출에
> 전달되지 않던 버그를 같이 고쳤다(이전엔 인자를 줘도 조용히 무시되고 `sellha.py` 자체 기본값이 적용됐다).
