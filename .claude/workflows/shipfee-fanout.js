export const meta = {
  name: 'shipfee-fanout',
  description: '배송비 추정 run 팬아웃 — 배치를 이미지예산+상품수 상한으로 묶어 Sonnet 워커에 분배',
  whenToUse: 'bulsaja-shipping-fee SKILL.md run(Workflow 모드)이 지정한 경우에만',
  phases: [
    { title: '추정', detail: '워커당 누적 이미지 ≤16장 · 상품 ≤6건 빈패킹 (기본 sonnet)' },
    { title: '오디트', detail: 'results 누락 배치 1회 재팬아웃' },
  ],
}
// args = { runDir: 절대경로, promptPath: 배송비-워커-프롬프트.md 절대경로,
//          model?: A/B 실측 전용 모델 오버라이드(기본 sonnet),
//          emphasis?: 워커 프롬프트 끝에 덧붙일 강조 문안(재팬아웃·A/B 용, 기본 없음),
//          batches: [{ n: 7, path: "...batch_007.json", imgs: 14, count: 3 }...]  // pending만
//          budgetImgs?: 16, maxCount?: 6, retried?: false }
//
// 스크립트는 파일시스템이 없다 — pending 산출은 호출자가 한다:
//   run_shipfee.py pending --run-dir <R>   → 이 args 를 그대로 찍는다.
...
// 상품 상한이 옵션정리(10)보다 낮다. 이 축은 상품당 이미지가 최대 5장(대표1+상세2+옵션2)이라
// 6건이면 이미 이미지 예산에 닿는다 — 상한을 높여도 빈패킹이 쓰지 못한다.
const MAXCOUNT = (args_.maxCount || 6)

// 빈패킹 — 큰 배치부터 first-fit. 배치의 imgs 는 상한이지 실열람이 아니다
// (지시서 §0 열람 규율이 실비용을 억제한다 — 텍스트에 치수가 있으면 0장으로 끝난다).
...
// 섞인다(다른 3벌과 같은 관례로 맞춘다). 없으면 프롬프트가 바이트 단위로 종전과 같다.
const EMPHASIS = args_.emphasis ? `\n\n[강조]\n${args_.emphasis}` : ''

function prompt(bin) {
  return `너는 불사자 배송비 추정 워커다. 지시서 ${args_.promptPath} 를 Read 하고 그대로 따른다.\n` +
    `맡은 배치 파일 (각각 §0부터 §4까지 독립 수행, 순서대로):\n` +
    bin.items.map(b => `  - 배치 ${b.n}: ${b.path}  (상품 ${b.count}건 · 이미지 ${b.imgs}장)`).join('\n') +
    `\n\n산출물은 ${args_.runDir}/results/result_NNN.json (NNN=배치번호 3자리 0채움).\n` +
    `배치 하나당 Write 1회. 금액(원)은 만들지 않는다 — 무게·치수·파손위험만. ` +
    `반환은 배치별 결과 배열(results)만.` + EMPHASIS
}

phase('추정')
// 이 팬아웃은 세션 기본 가이드라인(동시 에이전트 ≤15)을 스킬 지시로 덮어쓴다 —
// 워커는 디스크에만 쓰고 상호 간섭이 없다. 상한은 이미지예산과 하네스 동시성 캡이 관리한다.
const first = await parallel(bins.map((bin) => () =>
  agent(prompt(bin), {
    label: `배송비:${bin.items.map(b => b.n).join(',')}`,
    phase: '추정', schema: SCHEMA, model: MODEL, effort: 'low', agentType: 'fanout-worker',
  })))
...
return {
  ...
  워커: bins.length,
  완료배치: okSet.size,
  추정: ok.reduce((s, r) => s + (r.done || 0), 0),
  신뢰도낮음: ok.flatMap(r => r.held || []),
  오류: all.filter(r => r.error).map(r => ({ 배치: r.batch, 사유: r.error })),
  재팬아웃필요배치: stillMissing,
  다음: stillMissing.length
    ? `배치 ${stillMissing.join(',')} 미완 — run_shipfee.py pending 으로 다시 확인 후 재호출`
    : `run_shipfee.py apply --run-dir ${args_.runDir} 로 미리보기 → Claude 표본검수 → ` +
      `바로 --commit (승인 대기 없음). 변경폭 상한은 없다 — 차액 큰 건의 무게근거를 먼저 본다.`,
}
