...
- [ ] **ENG-08**: 불사자 계정(용팀장/용쌤)이 작업 단위로 격리되어 주입된다 — 작업 화면에 어느 계정으로 도는지 항상 표시된다
...
- [ ] **SAFE-03**: 자격증명(광고 시크릿·불사자 토큰)이 화면·로그·에러 메시지에 절대 나오지 않는다
...
- [ ] **BOARD-02**: 계정·규칙·상태로 거르고, 정렬하고, 검색한다 (계정 수를 화면·코드에 박지 않는다)
