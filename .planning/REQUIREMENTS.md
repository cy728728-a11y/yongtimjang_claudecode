...
> **✅ JOIN-02 마감 각주 (2026-09-21):** 기계 검증 전부 green(사유 3종 구분 · 광고그룹명 원문 ·
> 청소 목록 19그룹 883행이 원본 `adGroups` 와 정확히 일치 — CDP V-BOARD-20/21)에 더해,
> 열려 있던 수동 검증 한 줄이 **실사용으로 닫혔다.**
>
> 용팀장이 19그룹 중 **최난도 행**(`소형이동식오피스` · 번호 `—` · 사유 `추출실패` · 703행 =
> 미해소 883행의 79.6%)을 골라, 보드가 준 **광고그룹명 원문만 들고** 네이버 검색광고에서
> 그 그룹을 찾아 `25-2 소형이동식오피스` 로 이름을 고쳤다(`판매상품_17-3_오운웨이컴퍼니` 도 함께).
> 캠페인명 컬럼이 없는 것도 장애가 되지 않았다. 수정이 해소로 이어지는지 코드로 실측 확인:
> `market_number("25-2 소형이동식오피스")` → `25-2`, 불사자 마켓그룹에 `25-2` 존재(`5번_용쌤25-2` /
> groupId `1001822`), 해당 groupId 는 인덱스 미보유 → 재스캔 시 🔴(사람 몫)에서 🔵(기계 몫)로 이동.
> 두 배너 혼동 여부는 용팀장 진술 **"안헷갈려"** + 행동 증거(빨강 배너를 보고 실제로 광고를 고쳤다).
>
> **넘긴 것:** `판매상품_10-3_제이와이에프컴퍼니`(13행)는 네이버 광고그룹에 실물이 없어
> 용팀장이 넘기기로 결정. 보드 결함이 아니다.
> 전문: `03-HUMAN-UAT.md`(passed 2/2) · `03-VERIFICATION.md` §Human Verification 마감.
