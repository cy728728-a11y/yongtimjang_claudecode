...
- **D-02b: 번호로 마켓그룹을 못 찾으면 광고그룹명의 "몰 이름" 을 2차 키로 재시도한다 (2026-09-21 실측).**
  같은 스토어를 광고계정 둘이 서로 다른 번호로 돌리고 있다. 낮은 번호(`5-x~12-x`)는
  높은 번호 그룹의 **상품 부분집합**이다 — 실측 `mallProductId` 교집합:

  | 낮은 번호 | 짝 | 상품수 | 교집합 |
  |---|---|---|---|
  | 5-2 제이와이에이컴퍼니 | 16-2 | 10 / 1000 | 10 (전부) |
  | 6-1 온에이컴퍼니 | 17-2 | 15 / 784 | 15 (전부) |
  | 6-2 제이에스에이컴퍼니 | 18-2 | 18 / 1000 | 18 (전부) |
  | 7-1 오운웨이컴퍼니 | 17-3 | 224 / 1000 | 85 |
  | 7-2 제이제이이컴퍼니 | 19-2 | 12 / 758 | 12 (전부) |
  | 8-2 제이제이씨컴퍼니 | 21-2 | 9 / 745 | 9 (전부) |
  | 8-3 조흔가게 | 21-3 | 12 / 738 | 11 |
  | 10-1 온애이컴퍼니 | 20-3 | 4 / 785 | 4 (전부) |
  | 12-1 와이제이즈컴퍼니 | 13-3 | 17 / 767 | 17 (전부) |
  | 12-2 제이와이디컴퍼니 | 14-3 | 13 / 897 | 13 (전부) |
  | 5-3 제이와이전문몰 | 16-3 | 15 / 730 | 11 |

  **별칭표를 코드에 박지 마라.** 매 회차 광고그룹명 전체에서 `번호 → 몰이름` 을 뽑아
  **같은 몰이름을 가진 번호끼리 묶어** 자동 유도한다. 몰 이름이 한 글자 다른 별개 스토어가
  실제로 존재하므로(`오운웨이`/`오운왜이`, `온에이`/`온애이`/`오내이`) **정확 일치로만** 묶는다.
...
**계정 매핑 — 해소됐다 (2026-09-21 실측):**
불사자 계정은 **하나만 본다.** 부킹(=용쌤) 계정 마켓그룹 86개를 전수 확인했고 `5-x~12-x` 는 없다.
그런데 그게 별도 계정에 있는 게 아니라, **같은 스토어의 두 번째 광고그룹**이었다(D-02b).
따라서 **ENG-08 은 "표시" 로 충분하다 — 계정 전환은 필요 없다.**

**해상률 실측 (③+⑤ 194행):**

| 구분 | 행 | 비중 | 처리 |
|---|---|---|---|
| 번호가 부킹에 그대로 있음 | 92 | 47% | D-02 로 해소 |
| `소형이동식오피스` (번호 없음) | 57 | 29% | 용팀장이 광고그룹명 수정 예정 → 25-2. 그 전까지 미해소 |
| 낮은 번호 + 짝 있음 | 31 | 16% | D-02b 로 해소 |
| 낮은 번호 + 짝 없음 | 14 | 7% | **진짜 미해소** |

짝 없는 8개 그룹 — 어떤 광고그룹과도 `mallProductId` 가 겹치지 않는다:
`5-1 와이제이패밀리` · `6-3 리포플` · `7-3 레벨컴퍼니` · `8-1 오운왜이컴퍼니` ·
`9-1 오내이컴퍼니` · `9-2 제이제이디컴퍼니` · `10-2 제이와이쓰컴퍼니` · `10-3 제이와이에프컴퍼니`
→ D-14 대로 사유와 함께 미해소로 띄운다. 별도 처리 경로를 만들지 마라.

**D-02 검증 실측:** 광고 16-2 의 `신축 유리대차 고무바퀴 접이식대차 확장형` →
불사자 `16번_용쌤16-2` 에 정확히 1건. 높은 번호에선 번호 매칭이 깨끗하게 작동한다.

**광고그룹 전수 (2026-09-21 API 실측):** 7계정 63개 = 번호 있음 62 + 번호 없음 1(`소형이동식오피스`).
`9-3` `12-3` 은 광고 쪽에 아예 없다.
