...
불사자 계정은 **하나만 본다.** 부킹(=용쌤) 마켓그룹 86개 전수 확인했고 `5-x~12-x` 는 없다.
용팀장 계정에 있는 것도 아니다 — **광고 쪽 중복 오등록**이다(D-02b).
따라서 **ENG-08 은 "표시 + 거부" 로 충분하다. 계정 전환 로직은 범위 밖이다.**

**해상률 실측 (③+⑤ 194행, 2026-09-20 회차):**

| 구분 | 행 | 비중 | 성격 |
|---|---|---|---|
| 번호가 불사자에 있음 → 해소 | 92 | 47% | 정상 |
| `소형이동식오피스` — 번호 누락 | 57 | 29% | 광고 쪽 오등록. 용팀장이 25-2 로 수정 예정 |
| `5-x~12-x` — 중복 오등록 | 45 | 23% | 광고 쪽 오등록. 용팀장이 직접 삭제 |

**미해소 102행(53%)은 전부 광고 쪽 데이터 오류다. 조인 로직의 결함이 아니다.**
용팀장이 광고를 청소하면 해상률은 저절로 올라간다.

→ **JOIN-02 설계 요구:** 이 화면이 곧 용팀장의 **광고 청소 목록**이 된다.
미해소 사유에 **광고그룹명 원문과 추출된 번호를 같이** 띄워야 사람이 바로 행동한다.
예: `광고그룹 "판매상품_5-2_제이와이에이컴퍼니" 의 번호 5-2 가 불사자 마켓그룹에 없음`
→ **JOIN-01 설계 요구:** 해상률이 낮은 게 **버그가 아니라 청소 대상이 많다는 뜻**임을 화면이 오해 없이 전달해야 한다.

**D-02 검증 실측:** 광고 `16-2` 의 `신축 유리대차 고무바퀴 접이식대차 확장형` →
불사자 `16번_용쌤16-2` 에 정확히 1건. 번호 1:1 매칭은 깨끗하게 작동한다.

**광고그룹 전수 (2026-09-21 API 실측):** 7계정 63개 = 번호 있음 62 + 번호 없음 1.
