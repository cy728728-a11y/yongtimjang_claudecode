...
**⚠ 광고그룹 정리 완료 (2026-09-21 용팀장 직접 수행) — 아래 숫자가 정본이다.**
광고그룹 **69개 → 58개**. 중복 오등록 15개 삭제 + 4개 번호 정정.

| 정정 | 전 | 후 |
|---|---|---|
| 번호 누락 | `소형이동식오피스` | `25-2 소형이동식오피스` |
| 오번호 | `7-3 레벨컴퍼니` | `19-3 레벨컴퍼니` |
| 오번호 | `10-2 제이와이쓰컴퍼니` | `23-2 제이와이쓰컴퍼니` |
| 오번호 | `10-3 제이와이에프컴퍼니` | `23-3 제이와이에프컴퍼니` |

**번호 없는 광고그룹은 0개다.** `소형이동식오피스` 예외는 **사라졌다** — 예외 처리 코드를 쓰지 마라.
...
|---|---|---|
| 해소 | **152** | **78%** |
| 삭제된 광고그룹의 소재 (다음 회차엔 안 잡힌다) | 15 | 8% |
| 번호가 불사자에 없음 — 남은 오등록 5그룹 | 27 | 14% |

→ 다음 회차 실질 **179행 중 152행 = 85%**.

**남은 미해소 5그룹** (용팀장이 번호를 정해야 한다. 불사자에 `5-x`·`7-x`(1 제외)·`8-x`·`9-x` 마켓그룹이 없다):
`7-1 오운웨이컴퍼니`(16행) · `8-1 오운왜이컴퍼니`(7행) · `9-1 오내이컴퍼니`(2행) ·
`9-2 제이제이디컴퍼니`(2행) · `5-1 와이제이패밀리`(0행) — 전부 게재중.

**같은 번호를 쓰는 광고그룹 6쌍** — `20-1`(계정 2개) · `21-1`~`25-1`(`판매상품_` vs `1000개_`).
같은 번호 → 같은 불사자 그룹이라 조인은 동일하게 풀린다. **인덱스 구축 시 번호 기준 중복제거만** 하면 된다.

→ **JOIN-02 설계 요구:** 미해소 사유에 **광고그룹명 원문 + 추출된 번호**를 같이 띄운다.
이 화면이 용팀장의 광고 청소 목록이 된다.
...
**인덱스 모수도 바뀌었다 (RESEARCH §D-13 갱신):**
③⑤가 걸치는 마켓그룹 **31개 → 33개** (`23-3`·`25-2` 추가), 상품 합계 **58,778 → 70,548**.
정리로 해상률은 올랐지만 **인덱스 구축 비용은 늘었다** (3.7건/초 기준 4시간 25분 → **5시간 18분**).
비대 그룹 둘이 비용을 지배한다:

| 그룹 | 상품수 | ③⑤ 행 | 훑는 비용 |
|---|---:|---:|---:|
| `13번_용쌤13-2` | 28,230 | **8** | 2시간 7분 |
| `5번_용쌤25-2` | 10,990 | **57** | 49분 |
| 나머지 31개 | 31,328 | 87 | 2시간 21분 |

`13-2` 는 상품의 40% 를 먹고 8행만 돌려준다. **범위 결정이 필요하다 — Open Question.**
