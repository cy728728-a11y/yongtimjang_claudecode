...
DEL_SUBJ_ABSOLUTE = [
    ...
    # 용팀장 지시(2026-09-03): 전자세금계산서 발행 알림은 안 본다 — 홈택스에 원본이 있다.
    # 발행 알림의 정형 문구만 잡는다. 소명·수정요청·부가세 안내 등 다른 세무 메일은 KEEP 유지.
    r'전자세금계산서가 발행되었습니다',
]
...
DEL_SUBJ = [
    ...
    # 스마트스토어 주문현황 알림 (용팀장 지시 2026-08-31): 발신자가 smartstoreadmin_noreply.
    # '안내' 뿐 아니라 '알려드립니다' 형태로도 온다 — 둘 다 잡는다.
    r'주문(현황|상태).*(안내|알려드립니다)',
    # 네이버 검색광고 자동충전 알림 (용팀장 지시 2026-09-06): ads_noreply@navercorp.com.
    # 광고비 증빙은 광고시스템·세금계산서에 남는다. 충전 실패·잔액소진 경고는 문구가 달라 안 걸린다.
    r'자동충전하신 내역',
