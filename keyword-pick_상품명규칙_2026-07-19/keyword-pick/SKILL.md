...
- **⑤ `smartstore-seo-product-name` 스킬 (구현됨, 2026-07-19)**: `[01 추천키워드]` 5~6개(또는 불사자 키워드 도구 직접 조회)를 받아 핵심 상품명 2개(겹치는 단어 ≤1) + 추가 키워드(나머지 키워드의 미사용 단어)로 최종 상품명 1개 생성(term count ≤7·중복 단어 1개·2회) → 요청 시 불사자 적용+검증. 규칙 원본은 [`references/키워드&상품명작성에이전트.md`](references/키워드&상품명작성에이전트.md) §6~§10 (스킬 `references/상품명-작성-규칙.md`와 동일 내용). 전체 흐름은 [`references/keyword-pipeline-flow.md`](references/keyword-pipeline-flow.md).
